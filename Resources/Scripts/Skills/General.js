...
class HeavyHit extends Skill {
  ...
  onUse(source, target) {
    // 在被使用时触发的函数 （主动技能使用onUse 被动技能使用onLearned）
    const game = getGameInstance(); // 获取Game实例
    return game.insertEvent(
      // createEvent为插入执行一个事件
      ...
      game.eventWrapper(
        ...
        },
        {
          ...
          during: async (self, game) => {
            // during 时机应将所有所需的数据写入data，如无需要则不需写
            self.data.attackPower = await self.data.source.getNextAttribute(
              "strength"
            ); // 获取发起者的 strength 属性
          },
          after: async (self, game) => {
            // after 时机用于执行事件所有的功能（所需数据应从self.data获取）
            if (self.data.battle) {
              const next = self.data.battle.onDamaged(
                // 调用Battle类的onDamaged函数
                self.data.source, // 此函数会创建一个 onDamaged 事件
                self.data.target,
                self
              );
              const source = self.data.source.getPlayerName ? self.data.source.getPlayerName() : self.data.source.getName();
              const target = self.data.target.getPlayerName ? self.data.target.getPlayerName() : self.data.target.getName();
              next.data.skill = self.data.skill; // 为 onDamaged 事件添加将来需要调用的数据
              next.data.__attackPower = self.data.attackPower;
              next.addHook("after", async (self, game) => {
                // 为 onDamaged 事件添加after时机的钩子
                // 钩子在事件同时机之后执行
                const current = getUIInstance().getCurrentScene(); // 获取当前场景 Scene类 | SubScene类
                if (current.addBattleLog)
                  current.addBattleLog(
                    i18n.f("skill_use_text", {
                      Source: source,
                      SkillName: i18n.t(this.skillName),
                      DamageNumber: self.data.__attackPower,
                      Target: target
                    })
                  );
